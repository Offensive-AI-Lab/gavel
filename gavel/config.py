...
def load_config(config_path: str = "config.json") -> GavelConfig:
    ...
    model_cfg = ModelConfig(
        ...
        selected_layers_range=tuple(raw.get("model", {}).get("selected_layers_range", list(ModelConfig.selected_layers_range))),
    ) if "model" in raw else ModelConfig(
        # Legacy support: flat config structure
        name_or_path=raw.get("model_name_or_path", ModelConfig.name_or_path),
        selected_layers_range=tuple(raw.get("selected_layers_range", list(ModelConfig.selected_layers_range))),
    )
    
    training_cfg = TrainingConfig(
        batch_size=raw.get("training", {}).get("batch_size", raw.get("batch_size", TrainingConfig.batch_size)),
        batch_size_text=raw.get("training", {}).get("batch_size_text", TrainingConfig.batch_size_text),
        ...
        cleanup_embeddings=raw.get("training", {}).get("cleanup_embeddings", TrainingConfig.cleanup_embeddings),
    )
    ...
    rnn_cfg = RNNConfig(
        ...
        sequence_length=raw.get("rnn", {}).get("sequence_length", raw.get("RNN_sequence_length", RNNConfig.sequence_length)),
    ...
    paths_cfg = PathsConfig(
        ...
        calibration_dataset=raw.get("paths", {}).get("calibration_dataset", PathsConfig.calibration_dataset),
        labels_path=raw.get("paths", {}).get("labels_path", raw.get("labels_path", PathsConfig.labels_path)),
        ruleset_path=raw.get("paths", {}).get("ruleset_path", raw.get("ruleset_path", PathsConfig.ruleset_path)),
    )
    ...
    eval_cfg = EvalConfig(
        ...
        max_samples_per_usecase=raw.get("eval", {}).get("max_samples_per_usecase", EvalConfig.max_samples_per_usecase),
