...
representation extraction for training the GAVEL classifier.
"""
...
class CognitiveElementDataset(Dataset):
    """PyTorch Dataset for training cognitive element classifiers.
    ...
    def __getitem__(self, idx):
        ...
        prompt_ids = prompt_result.input_ids if hasattr(prompt_result, 'input_ids') else list(prompt_result)
            ...
            if filename.endswith(".json"):
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        logger.debug(f"Loaded {len(filtered_conversations)} conversations from {filename} into topic '{topic_name}'.")
...
def split_dataset_into_train_val(
    ...
    all_json_files = [f for f in os.listdir(dataset_root_path) 
    ...
    for file_name in all_json_files:
        ...
        try:
            ...
            if not isinstance(conversations, list):
                logger.warning(f"File {file_name} does not contain a JSON list. Skipping content split for this file.")
                continue
            ...
            if train_conversations:
                ...
                logger.debug(f"No training conversations generated for {file_name}. Skipping train file creation.")
            ...
            if val_conversations:
                ...
                logger.debug(f"No validation conversations generated for {file_name}. Skipping val file creation.")
            ...
            logger.warning(f"An unexpected error occurred processing {file_name}: {e}. Skipping this file.")
...
def load_model_and_tokenizer(model_name_or_path):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_name_or_path, padding_side="left", legacy=False, local_files_only=is_local
    )
...
def extract_per_sequence_reps(
    *,
    ...
    logger.debug(f"[extract] n_q_heads={n_q_heads} n_v_heads={n_v_heads} head_dim={head_dim} readout_dim={readout_dim}")
        ...
        with open(index_path, "a") as index_f:  # append if resuming
            for batch in loader:
                ...
                for layer_idx in selected_layers:
                    ...
                    r_layer = r_heads.permute(0, 2, 1, 3).reshape(B, L, readout_dim)  # (B, L, H_v*D)
...
class WindowedSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        for fi, fname in enumerate(self.files):
            ...
            reps = torch.load(fpath, map_location="cpu")  # (T, num_layers, readout_dim), stored as fp16
            T = reps.shape[0]
    ...
    def __getitem__(self, idx):
        ...
        reps = reps.reshape(T, L * D)  # flatten layer axis for the RNN input used by your current code
