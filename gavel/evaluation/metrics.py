...
def convert_labels_to_tensors(data: dict, labels_dict: dict) -> dict:
    ...
    for category, labels in data.items():
        ...
        processed_data[category]['all_required_labels'] = create_one_hot_tensor(labels.get('all_required', []))
        processed_data[category]['supporting_labels'] = create_one_hot_tensor(labels.get('supporting', []))

    return processed_data

def load_any_of_conditions(data: dict, labels_dict: dict[str, int]) -> dict[str, list[torch.Tensor]]:
...
def compute_triggers(
    ...
    if isinstance(logits_list, np.ndarray):
        ...
        raise ValueError("logits_list should be a numpy.ndarray or torch.Tensor of shape (num_windows, num_labels).")
...
def eval_usecase_detection(
    ...
    """
    logger.debug(f"Evaluating use case stats for dialogue: {dialogue_name}, use case: {gt_uc_name}, split {split}")
    ...
    def has_all_required(uc: str) -> bool:
        required_idxs = (all_usecase_gt_labels[uc]["all_required_labels"] == 1).nonzero(as_tuple=True)[0]
...
def compute_weighted_metrics(
    ...
    # Accuracy
    df['accuracy'] = (df['true_positive'] + df['true_negative']) / (df['true_positive'] + df['true_negative'] + df['false_positive'] + df['false_negative'])
...
def plot_usecase_metrics_table(df_metrics: pd.DataFrame, output_path: str):
    ...
    mask_neutral = df_plot["Usecase"].str.contains("conversational", case=False) | \
...
def evaluate_ce_detection_rules(
    ...
    if (req & any_of_union).any() or (supp & any_of_union).any():
        raise ValueError("A label appears in both any_of and all_required/supporting. Make sets disjoint or add precedence.")
    # 2) No overlap between all_required and supporting
    if (req & supp).any():
        raise ValueError("A label is marked both all_required and supporting. Make sets disjoint or add precedence.")
    ...
    if len(any_of_groups) > 1:
        ...
        if overlaps.any():
            raise ValueError("A label appears in multiple any_of groups. Make groups disjoint or add precedence.")
...
def evaluate_usecase_detection_from_logits(
    ...
    for meta_path, npy_path, meta in iterator:
        ...
        # Compute triggers
        triggers = compute_triggers(dialogue_logits, thresholds=setup["thresholds"], patience_rate=1)
...
def evaluate(
    ...
    setup = load_evaluation_setup(
        thresholds_path, unified_ruleset_path, labels
    )
    ...
    results["df_weighted_averages"].to_csv(output_path / "usecase_weighted_averages.csv", index=True)
