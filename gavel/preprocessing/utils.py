...
class DialogueDataset(Dataset):
    ...
    def __init__(self, conversations, tokenizer, max_length=None, llama_clean_template=True, strip_think_toks = True):
        ...
        self.prepend_user_role = self.is_mistral_like or self.is_gemma_like  # qwen doesn't need this
    ...
    def _segment_by_qwen_turns(self, ids):
        ...
        for si in s_idx:
            ...
            if self.strip_think and role == "assistant" and \
               self.Q_THINK_S is not None and self.Q_THINK_E is not None:
    ...
    def __getitem__(self, idx):
        ...
        input_ids, attention_mask, assistant_mask, spans = self._tokenize_with_assistant_mask(messages)
        return {
            "input_ids": input_ids,
...
def _normalize_to_conversations(data):
    ...
    if isinstance(data, list):
        ...
        if all(
            isinstance(x, dict) and ("content" in x or "Statement" in x) for x in data
        ):
...
def load_with_dataloaders(
    ...
    for filename in os.listdir(data_directory_path):
        ...
        try:
            ...
            logger.debug(
                f"Created dataloader for {os.path.splitext(filename)[0]}: {len(ds)} convs"
            )
...
def extract_logits_for_directory(
    ...
    for dialogue_id, dl in dataloaders.items():
        ...
        if max_samples is not None and processed >= max_samples:
            if logger:
                logger.debug(f"Reached max_samples limit ({max_samples}) for directory {usecase_path}")
            break
        ...
        for batch in dl:
            try:
                results = process_dialogue_batch(
                    batch, model, rnn_model, tokenizer,
                    selected_layers, window_size, window_stride
                )
...
def extract_logits_for_split(
    ...
    if usecases is not None:
        ...
        if logger:
            logger.debug(f"Filtered to {len(dirs_to_process)} directories matching usecases: {usecases}")
    ...
    iterator = tqdm(dirs_to_process, desc=f"Processing {split}") if show_progress else dirs_to_process
...
def extract_dialogues_in_memory(
    ...
    for split in splits:
        ...
        iterator = tqdm(dirs_to_process, desc=f"Extracting {split}") if show_progress else dirs_to_process
        ...
        for data_dir, usecase_path in iterator:
            ...
            for dialogue_id, dl in dataloaders.items():
                if max_samples_per_usecase is not None and processed_in_usecase >= max_samples_per_usecase:
                    break
                ...
                for batch in dl:
                    try:
                        ...
                        if logger:
                            logger.warning(f"CUDA OOM - skipping batch in {usecase_path}/{dialogue_id}")
                        torch.cuda.empty_cache()
                        continue
