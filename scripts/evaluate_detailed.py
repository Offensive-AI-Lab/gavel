...
def main():
    ...
    topic_stats = {k: {"true_positive": 0, "false_positive": 0, "true_negative": 0, "false_negative": 0} for k in LABELS.keys()}
    rulebased_topic_stats = {k: {"true_positive": 0, "false_positive": 0, "true_negative": 0, "false_negative": 0} for k in LABELS.keys()}
    ...
    for item in tqdm(eval_dialogues, desc="Evaluating", unit="dial"):
        ...
        split = meta.get("split", "") # Actually 'category' in extract_dialogues, need to check preprocessing mapping
        ...
        if split == "positive":
            ...
            gt_one_hot = None
            label_pt = Path(config.paths.eval_dataset) / "labels" / "positive" / gt_uc_name / f"{meta.get('dialogue_id', '')}.pt"
            if label_pt.exists():
    ...
    # CE stats by rules
    weighted_averages_ces_rulebased, df_weighted_averages_ces_rulebased = compute_weighted_metrics(rulebased_topic_stats)
    df_weighted_averages_ces_rulebased.to_csv(output_path / "weighted_averages_ces_rulebased.csv", index=True)
